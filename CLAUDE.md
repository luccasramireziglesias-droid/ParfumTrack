...
| Pro | $19.99/mes | Multi-perfil, backup auto, sync multi-dispositivo | **SIN CÓDIGO** |

⚠️ El plan Pro de $19.99 **no existe en el código**: no hay UI, no hay precio en Mercado
Pago (solo acepta `monthly` $9.99 y `annual` $95.88) y `isPro()` es un booleano —
tenés licencia o no. Hay dos planes reales: Free y Básico Pro. Un test lo verifica para
que nadie asuma que está a medio hacer.

## SEGURIDAD IMPLEMENTADA

- CSP restrictivo. `img-src` SIN `https:` (con XSS se exfiltraba por `new Image().src`). Sin Google Fonts (locales) ni OneSignal (no hay SDK cliente)
- `esc()` es para TEXTO; `escAttr()` para ATRIBUTOS. esc() no escapa comillas — eso causó un XSS explotable (BUG-26)
- Imágenes: `imgSrc()` valida el data URL COMPLETO, no el prefijo. SVG excluido
...
- Tests automatizados: 637 Vitest + 137 E2E Playwright (incluye `tests/fuzz.spec.js`: secuencias aleatorias con semilla fija que verifican invariantes de stock, cuotas, devoluciones, compras y reservas — subir volumen con `FUZZ_CORRIDAS=30 FUZZ_OPS=120`) — **los dos corren en CI** y frenan el deploy
