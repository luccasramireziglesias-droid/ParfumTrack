  ...
  updatePlanUI() {
    ...
    if (this.isPro()) {
      ...
      if (masStatus) masStatus.textContent = 'Básico Pro activo';
    } else {
  ...
  updateCuentaScreen() {
    ...
    if (isPro) {
      ...
      // El único plan pago se llama "Básico Pro": si el título dice
      // "Plan Pro" y el badge "Plan actual" está sobre "Básico Pro",
      // parece que son dos planes distintos y que ya tenés otro.
      title.textContent = 'Básico Pro';
  ...
  async registrarCuenta() {
    ...
    try {
      ...
      if (data.sent) {
        ...
        // El challenge que devuelve /trial es OBLIGATORIO al verificar: sin
        // él el backend rechaza el código antes de mirarlo. Guardarlo en
        // sessionStorage y no solo en memoria evita que recargar la pantalla
        // en el medio deje al usuario sin poder terminar el registro.
        this._otpChallenge = data.challenge || '';
        try {
          sessionStorage.setItem('pt_otp_challenge', this._otpChallenge);
          sessionStorage.setItem('pt_otp_email', email);
        } catch (_) { /* modo privado: seguimos con el valor en memoria */ }
  ...
  // Recupera lo pendiente del registro. En memoria primero; sessionStorage
  // como respaldo si el usuario recargó entre pedir el código y verificarlo.
  _getOtpPendiente() {
    let email = this._pendingEmail;
    let challenge = this._otpChallenge;
    if (!email || !challenge) {
      try {
        email = email || sessionStorage.getItem('pt_otp_email') || '';
        challenge = challenge || sessionStorage.getItem('pt_otp_challenge') || '';
  ...
  async verificarOTP() {
    ...
    const { email: pendingEmail, challenge } = this._getOtpPendiente();
    if (!pendingEmail || !challenge) {
      // Antes se mandaba igual y el backend devolvía "Datos inválidos", que
      // no le dice nada al usuario sobre qué hacer
      this.toast('Pedí un código nuevo para continuar', 'refresh');
      return;
    }
    ...
    try {
      const res = await fetch('/trial', {
        ...
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...this._getCsrfHeaders() },
        body: JSON.stringify({ step: 'verify', email: pendingEmail, otp, challenge, deviceId: this._getDeviceId() })
      });
      const data = await res.json();
      if (data.verified) {
        this._account = this._account || {};
        // pendingEmail y no this._pendingEmail: puede venir de sessionStorage
        this._account.email = pendingEmail;
        this._account.deviceId = this._getDeviceId();
        this.saveAccount();
        // El challenge es de un solo uso: no dejarlo dando vueltas
        this._otpChallenge = '';
        try {
          sessionStorage.removeItem('pt_otp_challenge');
          sessionStorage.removeItem('pt_otp_email');
        } catch (_) { /* modo privado */ }
